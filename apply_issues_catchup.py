#!/usr/bin/env python3
"""
apply_issues_catchup.py — fold the unapplied s21-deltas + s22 + s23 + s24
additions into ISSUES.md in one pass, bringing the register from s21 to
s24-complete.

Self-guarding:
  - idempotency sentinel: aborts if 'HELM-019' already present (already folded)
  - pre-state drift guard: aborts unless the file looks like the expected s21
    register (s21 stamp + '_None active._' + the old carried block present)
  - per-file timestamped backup before any write
  - DRY-RUN BY DEFAULT: prints a unified diff and writes nothing.
    Pass --apply to actually swap. Post-write readback re-asserts the new state.

Run from the repo root (where ISSUES.md lives):
    python apply_issues_catchup.py            # dry-run, shows the diff
    python apply_issues_catchup.py --apply    # writes, after backup
"""
import sys, os, time, difflib

PATH = "ISSUES.md"

NEW_CONTENT = r"""# HELM — Issues & Tech-Debt Register
...
def main():
    ...
    if "HELM-019" in cur:
        sys.exit("ABORT (idempotent): 'HELM-019' already present — looks already folded. No change.")

    # pre-state drift guard: must look like the expected s21 register
    expects = [
        "_Last updated: 2026-06-16 (s21)._",
        "### Bugs\n\n_None active._",
        "## Carried handover threads",
        "non-atomic open-snapshot wraps (overlaps HELM-003)",
    ]
    missing = [e for e in expects if e not in cur]
    if missing:
        sys.exit("ABORT (drift guard): the file is not the expected s21 pre-state. Missing anchors:\n  - "
                 + "\n  - ".join(repr(m) for m in missing)
                 + "\nRe-send `cat -n ISSUES.md` so the patch can be re-based.")

    if cur == NEW_CONTENT:
        sys.exit("ABORT: file already equals target content. No change.")

    diff = "".join(difflib.unified_diff(
        cur.splitlines(keepends=True),
        NEW_CONTENT.splitlines(keepends=True),
        fromfile="ISSUES.md (current, s21)",
        tofile="ISSUES.md (new, s24)",
    ))

    if not apply:
        print(diff)
        print("\n--- DRY RUN. Nothing written. Re-run with --apply to swap (a timestamped backup is taken first). ---")
        return
    ...
    with open(PATH, encoding="utf-8") as f:
        back = f.read()
    assert "HELM-019" in back, "post-write check failed: HELM-019 missing"
    assert "_Last updated: 2026-06-18 (s24)._" in back, "post-write check failed: stamp not s24"
    assert "_None active._" not in back, "post-write check failed: Bugs still empty"
    print(f"APPLIED. Backup: {bak}")
    print("Post-write checks passed: stamp s24, HELM-019 present, Bugs populated.")
