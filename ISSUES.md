...
_Last updated: 2026-07-01 (s45 — **HELM-032 resolved** + hotfix: deep-view Break-even & Stop reads `stop_loss_multiplier` from `strategy_settings` via `StrategySettings.get` (mirrors core 2×) — `367112c` / `9c921ed`. **HELM-043 registered** (condor health ignores greeks + proximity, `326adf8`). **CWAN removed** from watchlist (delisted; universe 206→205). **HELM-031 design settled**: shadow-capture, not go-live. Anthropic API key rotated + consolidated to `~/.helm/env`.)_
...
- **Next highest-leverage:** **HELM-031** — build the **shadow-capture** "-50% would-fire" flag for LONG_DEBIT (informational on the real book, no auto-close), agreed s45 after the backtest showed the live evidence too thin/noisy to auto-act. It rides the existing 3×/day REAL `helm check`. Then **HELM-042** (mean-reversion → momentum bias re-base, staged paper validation) as a deliberate sit-down. Also open: HELM-033 opt-in (`?book=paper|all` + REAL/PAPER page indicator).
- **Last shipped (s45):** HELM-032 resolved + hotfix — deep-view stop now reads the 2× lever from `strategy_settings` (`367112c`, scope hotfix `9c921ed`; live-confirmed on `RKLB --deep` → "2x Stop loss"). Also: HELM-043 registered (`326adf8`), CWAN removed (universe 206→205), and the exposed Anthropic API key rotated + consolidated to `~/.helm/env` across all four launchd agents + `.zshrc`.
- **Blocked (market/RTH):** largely cleared s45 — IVR up to date, CWAN removed, and the 3 REAL condors' `/health` core bands lit (validated live off the 13:28 REAL `helm check`, 4/4 GOOD legs each). Remaining: live re-pull of the deep-ITM spec CSPs (RKLB/IREN/OKLO/IONQ).
- **Counts:** 14 active (10 OPEN · 4 DEFERRED) · 7 parked · last shipped s45 (HELM-032).
