...
_Last updated: 2026-07-03 (s54 — **HELM-049 resolved** (paper→signal linkage): paper opens now stamp `positions.signal_id` from the in-scope scan signal (non-destructive — never consumes the signal), and the close-time outcome back-prop is REAL-gated so paper links can't contaminate signal outcomes. Forward-only (existing 136 paper stay unlinked). Next: HELM-033 (book-blind /health map). Origin at `c970499` (this checkpoint commit follows).)_
...
- **Next highest-leverage:** **HELM-033** — the book-blind CSP-only `/health` map commingles REAL + PAPER, corrosive to the learning substrate (the paper book is HELM-023's corpus; a surface that mixes it with real muddies what you learn from). Separate the books at the source. Then **HELM-023 Track A** (exit-lever scorecard) once more corpus accrues.
- **Last shipped (s54):** HELM-049 resolved — paper→signal linkage. `_paper_generate` now stamps `positions.signal_id` from the in-scope `sig['id']` after each booking (non-destructive: position-only write, never consumes the signal, so real opens still link); `close_cmd` back-prop is REAL-gated (`pos.book == 'REAL'`) so paper closes don't write outcomes onto signals. Two files, one commit, gate-first. Self-tested (paper close skips back-prop, real fires). Forward-only.
...
- **Counts:** 17 active (11 OPEN · 6 DEFERRED) · last shipped s54 · origin at `c970499` (this checkpoint commit follows).
