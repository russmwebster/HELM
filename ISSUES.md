...
_Last updated: 2026-06-30 (s43 — **HELM-041 build + scan investigation**. Resolved HELM-034 (stop-arm `cr_2x`/`cr_3x` are live via CSP, not dead branches — 337c8d4). Shipped HELM-041 per-leg mark store: `leg_checks` schema + live-DB migration (7c12122) and the paper writer (`manage_paper_book` persists GOOD per-leg marks when `book_live`, 8ae1d8b); split cleanly from HELM-029 (now just the dataclass refactor). Registered HELM-042 (scan bias scorer: mean-reversion → momentum re-base, 53e3d3d) after a read-only model showed the legacy oversold scorer + IVR<35 gate structurally mutes long calls. 4 commits local — push pending.)_
...
- **Phase:** scaffolding complete (live · paper · edge). `schema.sql` faithfully builds live incl. constraints/defaults/FKs (HELM-002, guarded by `diag_schema_constraints.py`); hot `positions` table indexed live (HELM-021). Decision core (HELM-027) is the active build — one verdict engine reading `strategy_settings`; single-leg `/health` maps (CSP · LONG_CALL) wired, multi-leg gated on the per-leg mark store. That store (`leg_checks`, HELM-041) now exists: schema + live DB + paper writer landed s43; the multi-leg `/health` band (`_core_band_ml`) + `check_cmd` real-book marking remain. Learning loop still the frontier — corpus accumulating on `core_v1`.
- **Next highest-leverage:** **finish HELM-041 — `check_cmd` real-book per-leg marking + step-3 `_core_band_ml` in `health.py`** — the WS5c payload that lets multi-leg positions (iron condor / bear put) reach the decision core. That unblocks the parked multi-leg `guidance_*` removal and feeds cleaner closed-trade data to HELM-023. Queued design: HELM-042 (momentum scan re-base — staged + paper-validated, not gut-flipped). HELM-029 is now scoped narrowly (the `Position`/`Leg` dataclass refactor only; the mark-store split to HELM-041).
- **Last shipped (s43):** HELM-041 step-2 paper writer — `manage_paper_book` persists GOOD per-leg marks to `leg_checks` when `book_live` (paper-only, live-only, 8ae1d8b). Session also: HELM-034 resolved (337c8d4), HELM-041 `leg_checks` schema + live-DB migration (7c12122), HELM-042 registered (53e3d3d). 4 commits local — push pending. Prior: s42 HELM-040 notify REAL-book filter (aea8bda).
- **Blocked (market/RTH):** `core_v1` IVR backfill for the 12 new names (run `helm ivr refresh` early — else they score `ivr_unknown`); live re-pull of the deep-ITM spec CSPs (RKLB/IREN/OKLO/IONQ) for the willing-to-own calls. New: first `leg_checks` rows land on the next RTH `com.helm.paper.manage` pass (post-16:00 all marks frozen → `book_live` False → nothing persists, by design); validate then via a `leg_checks` select.
- **Counts:** 15 active (11 OPEN · 4 DEFERRED) · 6 parked · last shipped s43 (HELM-041 step-2 paper writer, 8ae1d8b).
- **Next RTH (Wed 7/1):** run `helm ivr refresh` early for the 12 new `core_v1` names (else `ivr_unknown`); confirm the first `leg_checks` rows wrote from the paper-manage pass and spot-check them. HELM-019 stale-mark P&L self-heals once live marks return; the s24 no-close-off-stale-marks guard stands.
...
_s44 (7/1): longcall/icondor/bearput gathers now REAL-default too (mirrors `gather_csp`) — commingling closed for all strategies on `/health`. **Narrowed.** Remaining: opt-in paper param (`?book=paper|all` through `render` + `helm-server.py`) + a REAL/PAPER page indicator; the CSP-only-not-whole-book point stands._
