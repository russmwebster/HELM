...
_Last updated: 2026-07-03 (s52 — **HELM-023 Track B shipped** (VRP entry capture): `capture_entry_snapshot` now stamps `iv_rank`/`iv_percentile` at open via `IVHistory.latest(ticker)` (mirrors `close_snapshot`), so the variance-risk-premium entry lever is scoreable going forward. Forward-only — the 79 prior closes stay null. Gate cleared (HELM-005 resolved; 79 paper + 32 real closed with outcomes); HELM-023 DEFERRED→OPEN. Next: Track A exit-lever scorecard. Origin at `c4f415a` (s51 + this checkpoint pending push).)_
...
- **Next highest-leverage:** **HELM-023 Track A** — read-only exit-lever scorecard: risk-adjusted expectancy by `exit_reason` (DTE_MANAGE 55 / PROFIT_TARGET 15 / STOP 9), by strategy family, by holding period, over the 79 paper closes — tests the 21-DTE / over-management thesis and feeds HELM-030. Honest about n (STOP arm = 9 → direction, not significance; coarse cuts only). **Open modeling choice:** risk-adjusted expectancy as mean(pnl / max_loss) vs a dispersion-aware mean/std. Also open: **HELM-043 v1b** (RED act-reason) and **HELM-042** (momentum bias re-base).
- **Last shipped (s52):** HELM-023 Track B — VRP entry-capture fix. `capture_entry_snapshot` gains a `ticker` param and stamps `iv_rank`/`iv_percentile` from `IVHistory.latest(ticker)` (206-name coverage, refreshed daily; both open call sites pass ticker). INSERT rebalanced 23→25 cols/placeholders/values, AST + balance guarded. Verified live: IVHistory returns real ranks for open names (AAPL 71, AMD 100, ABT 98). Forward-only; the VRP corpus accrues from the next open.
...
- **Counts:** 16 active (10 OPEN · 6 DEFERRED) · last shipped s52 · origin at `c4f415a` (s51 + this checkpoint pending push).
