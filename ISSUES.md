...
_Last updated: 2026-07-03 (s51 — **HELM-027 closed (verdict-unified)**: the two dead multi-leg deep renderers (`guidance_icondor`/`guidance_bearput`) removed; the decision core is the single verdict source across every family. The demoted evidence layer (assess_position / generate_guidance / composite score_*) stays by design — tracked as HELM-048 (revisit only on visible drift). Next: scope HELM-023 (the learning endgame), gated on HELM-005 breadth. Origin at `c4f415a`; this checkpoint commit follows.)_
...
- **Next highest-leverage:** **HELM-023 scoping** — the learning / look-back layer (the endgame): score the PAPER counterfactual corpus for risk-adjusted expectancy, separating entry-lever from exit-lever learning. First confirm the gate — HELM-005 breadth landed + closed paper trades to score (the s48 corpus is 73/73 with outcomes, so the trades half looks met). Other open builds: **HELM-043 v1b** (RED act-reason for a breached short strike) and **HELM-042** (momentum bias re-base).
- **Last shipped (s51):** HELM-027 closed (verdict-unified) — removed the two dead multi-leg deep renderers (`guidance_icondor`/`guidance_bearput`, 0-ref, superseded by `_core_band_ml` + the s50 proximity signal). The decision core (`evaluate` / `_core_band_*` / `band_for`) is now the single verdict source across every family; the composite/guidance code remains demoted-to-evidence by design (HELM-048).
...
- **Counts:** 16 active (9 OPEN · 7 DEFERRED) · last shipped s51 · origin at `c4f415a` (this checkpoint commit follows).
