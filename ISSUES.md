...
_Last updated: 2026-07-03 (s53 — **days_to_earnings capture wired** (HELM-023 capture-completeness): `capture_entry_snapshot` now stamps `days_to_earnings` from `earnings_state(ticker)` (watchlist cache, no network) — same in-writer shape as the iv_rank fix. Forward-only. Two follow-ups logged: HELM-049 (paper `signal_id` linkage) + HELM-050 (earnings cache coverage 65/205). Next: HELM-049. Origin at `6db1d93` (this checkpoint commit follows).)_
...
- **Next highest-leverage:** **HELM-049** — paper `signal_id` linkage (the selection-skill pick-linkage HELM-023 needs). Paper is 0/136 linked because `link_position_opened` is REAL-gated by design (a paper link would *consume* the signal and starve a later real open). Needs a non-destructive paper association: stamp `positions.signal_id` from the matching signal without mutating its real-book fields, recording strategy match/no-match. Then **HELM-033** (book-blind /health map), then **HELM-023 Track A** (exit-lever scorecard).
- **Last shipped (s53):** days_to_earnings entry capture (HELM-023 capture-completeness). `capture_entry_snapshot` computes `days_to_earnings` from `earnings_state(ticker)` (watchlist cache, no network) when the caller didn't supply it — single insertion after the iv_rank block; the field was already a param + INSERT column. Verified live (AAPL 27, ABT 13, AMD 32; uncovered → None). Forward-only. Flagged: watchlist earnings coverage 65/205 (HELM-050).
...
- **Counts:** 18 active (12 OPEN · 6 DEFERRED) · last shipped s53 · origin at `6db1d93` (this checkpoint commit follows).
...
**HELM-027 · `DESIGN` · `RESOLVED (verdict-unified s51)` · Decision core — unified verdict engine**
...
**HELM-049 · `DESIGN` · `OPEN` · Paper positions unlinked to their originating signal (selection-skill gap)**
_Paper positions are 0/136 linked to a signal (real 15/55). `link_position_opened` (`signal.py`) is REAL-gated at both open sites (`entry_snapshot.py:232` and the multileg sibling) by design: it *consumes* the signal (sets `position_opened` / `position_id` / `russ_action='OPEN'`, matching the latest *unlinked* signal), so a paper link would starve a later real open on the same ticker. HELM-023's selection-skill scoring needs the paper pick-linkage → wants a **non-destructive** paper association: resolve the matching signal for the ticker and stamp `positions.signal_id` on the paper position WITHOUT mutating the signal's real-book fields, ideally recording whether the paper strategy matched the signal's `top_strategy` (pick vs counterfactual). Design task, not a wire-in._
