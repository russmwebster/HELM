...
_Last updated: 2026-06-29 (s41 — **HELM-027 WS5b ship session**. LONG_CALL `/health` map now reads the decision core via sibling `_core_band_lc`; single-leg `/health` maps (CSP + LONG_CALL) are core-wired. Verified live: GOOD-only/RTH-only writer (29 GOOD/RTH checks, zero leakage) and the first stop-A/B managed pass (114 arm rows, natural-exit stamper, no false triggers). Surfaced + carried a PFE PAPER strategy/leg mismatch.)_
...
- **Next highest-leverage:** **HELM-027 WS6 — schedule `manage_paper_book()` on a launchd timer; confirm `_decision_capture` feeds the corpus.** Pairs with the now-live stop-A/B (first managed pass recorded today). WS5b shipped s41 (LONG_CALL `/health` → core via `_core_band_lc`; 4 REAL long calls core-banded, AAPL composite 24 → core YELLOW not RED). WS5c (multi-leg icondor/bearput → core) stays gated on per-leg marks. One sweep to remove dead `guidance_*`/composite-verdict funcs once all strategies wire.
- **Last shipped (s41):** HELM-027 WS5b — LONG_CALL `/health` map → core verdict (sibling `_core_band_lc`; `gather_longcall` +account_id/+greeks_source; `score_longcall`/`guidance_longcall` demoted to evidence; no-mark → UNKNOWN). `health.py` only — CSP path, `decision.py`, `verdict.py` untouched. Code+register paired (f7736fe), not yet pushed. Prior chain: s40 checkpoint (a47e671), HELM-038 Gap 1 (433c502).
...
- **Counts:** 14 active · 6 parked · last shipped s41 (HELM-027 WS5b — LONG_CALL /health → core, f7736fe).
...
- **WS6** [done · 6/30] — `manage_paper_book()` scheduled on launchd agent `com.helm.paper.manage` (Mon–Fri 10:00/12:30/15:45 ET, full HELM-037 cadence; `launchctl print` state=active, runs=9, exit 0). Corpus fed via `_record_arms`/`_close_arms` → `stop_arm_events`. (NB `_decision_capture`/`persist_scan_signals` is the *scan-signal* path — separate trigger, not the paper-manage writer.)
- **WS7** [done · 6/29] — book-live gate live in `manage_paper_book`: `evaluate()` stays verdict-only; CLOSE DEFERs when book not IBKR-live (`if not book_live`, paper_manage.py:102). Pairs with the completeness skip-gate.
