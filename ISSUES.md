...
_Last updated: 2026-06-27 (Sat — **s39 checkpoint**. Shipped the HELM-037 live-only persistence gate (`save_check` writes only GOOD/live marks; frozen/partial shown but not persisted, so the ~46%-frozen pollution stops at source; 228f995). This session also: characterized the HELM-035 sweep (systemic/time-bounded), logged HELM-036 + HELM-037, scoped 037 to RTH-only, locked its writer = manage pass at 10:00/12:30/15:45 live-only, re-timed `check.daily` 4×→3× + removed its notify text (plist reloaded), and flipped `stop_ab_active`→'1' (HELM-030 stop A/B now LIVE, freshness-gate confirmed). Full-book triage off Fidelity (16 positions, −\$50.6k unrealized ≈ 5.6% of IRA; WELL booked closed; deep-ITM spec CSPs = willing-to-own calls). Next: HELM-036 accessor build + remainder of 037.)._
...
- **Next highest-leverage:** **HELM-036 accessor build (read side).** One shared `latest_good_check` + `daily_good_series` that filters to GOOD rows and folds in the `_pnl_pick` bound; route all consumers through it (`/health` ×3, `status`, `analyze`; `notify` already removed via the check.daily change). Pairs with the HELM-037 remainder (consolidate to the single manage-pass writer + make ad-hoc `helm check` read-only). Also handles historical cleanup by construction — frozen rows + the 152 HELM-035 rows are never selected (filter at read; no destructive purge). Write-side live-only gate already shipped (228f995); stop A/B LIVE + seeding. Detailed enough to start the 036 build cold in a fresh chat.
- **Last shipped (s39):** HELM-037 live-only persistence gate in `save_check` (228f995) — only GOOD/live marks persist; py_compile-validated, committed + pushed. Plus this session's chain: HELM-036 + HELM-035-sweep (2274bc9), HELM-037 log (f27d6cf), RTH-only scope (ace72e2), writer+cadence lock (01d3831). Ops: `stop_ab_active`→'1' (DB); `check.daily` re-timed 4×→3× + notify removed (plist, launchctl-reloaded).
...
- **Counts:** 14 active · 6 parked · last shipped s39 (HELM-037 live-only gate, 228f995). This session: HELM-035 characterized, HELM-036/037 logged, 037 gate shipped, stop A/B activated.
