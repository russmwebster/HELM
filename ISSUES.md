...
_Last updated: 2026-07-04 (s58 — **HELM-042 momentum re-base flipped live**: scan directional bias is now momentum (MA stack + MACD + OBV, ADX-gated), legacy kept as a display shadow, missing-IVR names flagged rather than assessed; surfaced an IVR-coverage gap on liquid large-caps)._
...
- **Next highest-leverage:** no single big rock, and the frontier (HELM-023 Track A) stays **held on corpus maturity, not effort** — the exit-lever scorecard needs real DTE_MANAGE/STOP closes (0 genuine STOP + 7 DTE_MANAGE; the rest was seed artifact, now quarantined), which only accrue over calendar time. Of the ungated candidates, **HELM-030** (2x-credit stop inert at live position sizes) is the most substantive; then the newly-surfaced **IVR-coverage gap** (blank IVR on liquid large-caps, data-source — carried s58), **HELM-050** (earnings-cache cadence), and **HELM-033** (paper opt-in + book indicator on /health). HELM-042 is fully retired — the dead `iv_pct` path in `bias_to_strategy` (always-None arg → always-False iv_* flags) is gone, behaviour-preserving (175-case equivalence fixture green, s59).
- **Last shipped (s58):** HELM-042 scan bias re-based mean-reversion → momentum and flipped live — MACD/OBV/ADX added to `fetch_technicals` + `momentum_bias` rebuilt to the four-category ADX-gated v2 (`e1fe93d`); v2 promoted to the operative bias with legacy kept as a display shadow (`47e8912`); unknown-IVR guard so missing-IVR names show "⚠ IVR missing" instead of a made-up strategy, kept visible/ranked and excluded from paper booking (`d92fcd1`). Prior (s57): HELM-043 v1b RED condor headline (`a2c7802`).
...
- **Counts:** 17 active (11 OPEN · 6 DEFERRED) · last shipped s58 · HEAD at `d92fcd1` with 3 unpushed s58 commits (`e1fe93d` · `47e8912` · `d92fcd1`).
