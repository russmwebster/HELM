...
- **Next highest-leverage:** **HELM-036 Stage 4b + notify check (the 036 cleanup tail).** Stages 1–4a shipped — GOOD filter live across `status` / `analyze` / `/health`, `_pnl_pick` consolidated. Remaining: (4b) bound the csp/longcall P&L displays in `health.py` (they still render unguarded `pnl_unrealized`, and the CSPs IREN/EOSE/SMR/FCX/APH/INTC are HELM-035 victims, so corrupt values can surface) — needs a column-availability read of `gather_csp` / `gather_longcall` first (do they have `greeks_source` + `max_loss` / `max_profit` in hand?). (notify) confirm `notify.py:58` is actually dead (Status said it was de-wired from `check.daily` in s39) before patch-vs-resolve-by-removal. Both smaller than 4a; open the read-pass with the bridge freshly reopened and greps scoped narrowly.
...
- **Counts:** 14 active · 6 parked · last shipped s40 (HELM-036 Stage 4a, 8edb8f6). HELM-036 stays OPEN (Stage 4b + notify remain). This session: HELM-036 Stages 1–4a shipped; HELM-035 register premise corrected (15 GOOD corrupt rows → bound is load-bearing).
