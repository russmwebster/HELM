...
_Last updated: 2026-06-26 (s38 + audit addendum — post-checkpoint, pre-RTH. Ran the HELM-035 sweep: **systemic** (152 corrupt rows / 7 credit positions), **time-bounded** (fix 2026-06-18, none after), both `pnl_unrealized` + `pnl_pct` corrupt. Code audit of `checks` consumers → logged **HELM-036** (6 sites read the latest row with no GOOD/live/RTH filter; frozen marks leak into /health + notify = live correctness bug; fix = a shared `latest_good_check` accessor). Shelved the story strip as a blocked consumer of that accessor. STILL PENDING from s37: flip `stop_ab_active` → '1'. Next (RTH): live re-pull → WELL roll at 21-DTE; then build the accessor.)._
...
- **Counts:** 13 active · 6 parked · last shipped s37 (no s38 ship; HELM-035 characterized + HELM-036 logged at the 6/26 audit).
