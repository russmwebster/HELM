...
- **Next highest-leverage:** **HELM-027 WS5b - LONG_CALL /health map -> core verdict.** WS5a shipped 6/29 (CSP map reads decision.evaluate via helm.verdict; composite demoted to evidence; HELM-033 REAL-default on gather_csp; live-smoked, 9/10 differ from composite). WS5b: same single-leg pattern for gather_longcall (already selects current_price) - extend _core_band for LONG (max_loss = premium, upside uncapped), swap the longcall card/counts/sort to core. WS5c (multi-leg) stays gated on per-leg marks. Parked: one sweep to remove dead guidance_*/composite-verdict funcs once all strategies wire; compose the ITM/assignment caveat into the DTE_MANAGE headline.
- **Last shipped (s40):** HELM-038 Gap 1 — `confirm_spread` wired to the proven `open_multileg_with_snapshot` (credit verticals: BEAR_CALL/BULL_PUT record as one 2-leg position via select → contracts → net-credit, short leg absorbs the net override). Code+register paired (433c502), pushed. Prior s40 chain: HELM-036 Stages 1–4b (c5207d3→8edb8f6, 34fd34d) + notify GOOD-filter; verdict-layer lift (2d84e40); WS5a CSP /health→core (51a33d2/a61bed6).
...
- **Counts:** 14 active · 6 parked · last shipped s40 (HELM-038 Gap 1 — confirm_spread wired to the multi-leg writer, 433c502). HELM-036 RESOLVED to log at s40 checkpoint (GOOD-filter mandate complete across all six consumers: status · analyze×2 · health×3 · notify). This session: HELM-036 Stages 1–4b + notify; WS5a (CSP /health → core); HELM-038 surfaced → both gaps logged + corrected → Gap 1 fixed for credit verticals. HELM-035 residual (notify pnl-bound) still open.
...
- **WS5a** [done · 6/29] - CSP `/health` map verdict from `decision.evaluate` (shared `helm.verdict` adapters + new `_core_band`); composite `/100` + `s_*` cells demoted to evidence; no-mark → UNKNOWN; HELM-033 REAL-default on `gather_csp`. **WS5b** [next] - same for LONG_CALL (single-leg, `gather_longcall` has `current_price`). **WS5c** [gated] - multi-leg (icondor/bearput) blocked on per-leg marks. Composite-as-evidence leaves `guidance()`/`guidance_*` dead → one removal sweep when all strategies wire.
