...
_Last updated: 2026-06-26 (s38 + audit addendum — post-checkpoint, pre-RTH. Ran the HELM-035 sweep: **systemic** (152 corrupt rows / 7 credit positions), **time-bounded** (fix 2026-06-18, none after). Code audit of `checks` consumers → **HELM-036** (6 sites read the latest row, no GOOD/live/RTH filter; frozen marks leak into /health + notify = live correctness bug; fix = shared `latest_good_check` accessor). Logged **HELM-037** (persistence discipline: ad-hoc + `/health` read-only, a close-anchored scheduled spine is sole writer and persists only live marks — keeps `checks` clean by construction; partner to HELM-036). Shelved the story strip as a blocked accessor-consumer. STILL PENDING from s37: flip `stop_ab_active` → '1'. Next (RTH): live re-pull → WELL roll at 21-DTE; then build the accessor + persistence split.)._
...
- **Counts:** 14 active · 6 parked · last shipped s37 (no s38 ship; HELM-035 characterized + HELM-036/037 logged at the 6/26 audit).
...
_Logged 6/26 (partner to HELM-036). Today every check writes to `checks` regardless of trigger or mark quality — that's how frozen/stale and corrupt rows accumulate. Reframe the boundary as **looking vs recording**, not ad-hoc vs scheduled: the 6/26 $273.88 stale mark came from a *scheduled* after-hours run, so an ad-hoc-only exclusion wouldn't have caught it. Two rules: (1) ad-hoc `helm check` and `/health` are **read-only** — compute + display (live, or frozen-but-labeled), never write; (2) a scheduled spine is the **sole writer** and persists **only when marks are live** — if stale at fire time, log 'skipped, stale' rather than write a frozen duplicate (a frozen check is just the last live value repeated — no new info). Cadence: anchor on the **market-close** mark (canonical daily price; also fixes the 'where did it close' gap that bit us 6/26); optional midday slot only if HELM-023 later wants intraday shape (entry→outcome learning likely doesn't); **no after-hours runs** (they can only produce frozen rows). Result: `checks` becomes a clean, regular, live-only panel **by construction** — the source HELM-023 needs. Composes with HELM-036's accessor as belt-and-suspenders (the accessor still filters for the rare scheduled-but-stale case, e.g. IBKR down at close). Forward-looking only — does NOT clean existing rows (HELM-035 quarantine + a possible one-time frozen-duplicate purge are separate)._
