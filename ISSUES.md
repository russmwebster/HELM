...
_Last updated: 2026-07-03 (s49 — **HELM-038 Gap 2 shipped**: activity-import spread grouping — imported credit verticals + iron condors now persist as single multi-leg positions (real trade date + Fidelity provenance) instead of fragmenting into misclassified single-leg rows. New `activity_grouping.py` capability + `open_multileg_with_snapshot` `opened_at`/`notes` passthrough (2a) + `activity_cmd` grouping pass (2b). HELM-038 narrowed to DEFERRED (debit + diagonal ingestion). Origin at `db301ba`; this checkpoint commit follows.)_
...
- **Next highest-leverage:** **HELM-043** — condor health greeks (short-strike delta / proximity / theta·vega·gamma + trajectory). With multi-leg *ingestion* now closed on both surfaces (`helm open --confirm` and `helm activity` import), the multi-leg *monitoring* gap is the next structural weak point: condor `/health` verdicts are still P&L-only with null greeks. Then **HELM-042** (mean-reversion → momentum bias re-base). Remaining HELM-038 tail (debit verticals + diagonals) stays deferred.
- **Last shipped (s49):** HELM-038 Gap 2 — activity-import spread grouping. `activity_grouping.py` (pure grouping + credit-vertical/iron-condor inference, 27 fixtures green); `open_multileg_with_snapshot` `opened_at`/`notes` passthrough (patch 2a, Gap-1 callers unaffected); `activity_cmd` grouping pass before the single-leg loop (patch 2b). Imported credit verticals + iron condors persist as ONE multi-leg position with real trade date + Fidelity provenance; unrecognized legs fall through unchanged.
...
- **Counts:** 16 active (10 OPEN · 6 DEFERRED) · last shipped s49 · origin at `db301ba` (this checkpoint commit follows).
