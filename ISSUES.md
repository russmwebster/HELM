...
_Last updated: 2026-06-27 (Sat — shipped the HELM-037 **live-only persistence gate**: `save_check` now writes a check row only when marks are GOOD/live; frozen + partial reads are displayed but not persisted, so the ~46%-frozen pollution stops at the source. Also today: re-timed `check.daily` 4×→3× (10:00/12:30/15:45) + removed its notify text; locked HELM-037 writer = manage pass; flipped `stop_ab_active` → '1' (stop A/B live). Remaining for 037: single-writer consolidation + ad-hoc read-only. Next: HELM-036 accessor.)._
