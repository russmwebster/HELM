...
_Last updated: 2026-07-03 (s56 — **repo + register hygiene**: archived 104 spent one-off scripts from repo-root (112→8 files; load-bearing + git-tracked kept); refreshed the stale Phase + Next-RTH lines; migrated the resolved HELM-027 block out of Active (the Resolved log already carries it). No functional change. Origin at `6ac30c7` (this commit follows).)_
...
- **Next highest-leverage:** no single big rock — the endgame (HELM-023 Track A) is **held on corpus maturity, not effort**: the exit-lever scorecard needs real DTE_MANAGE/STOP closes, and there are currently 0 genuine STOP + 7 DTE_MANAGE (the rest was seed artifact, now quarantined). The auto-manager accrues these over calendar time. Near-term candidates are all smaller: **HELM-033** (paper opt-in + book indicator on `/health` — its commingling core is already closed at s44, so this is polish, not substrate-risk), **HELM-050** (earnings cache refresh cadence), **HELM-042** (momentum bias re-base), **HELM-043 v1b** (RED act-reason).
- **Last shipped (s55):** seed bulk-close quarantine + Track A finding. Recon of the 79-close paper corpus found 62 were a 6/25–6/26 administrative bulk-close (1–3 day holds; DTE_MANAGE/STOP labels impossible at those durations). Tagged those 62 `seed_bulk_close_20260626` via a guarded DB write (read-only probe → WAL-safe `.backup()` validate → timestamped backup → apply → readback; non-destructive, tags-only). Learnable corpus now 18 genuine closes (0 real STOP). Track A held pending maturity.
...
- **Counts:** 17 active (11 OPEN · 6 DEFERRED) · last shipped s55 · origin at `56658a7` (this commit follows).
...
**HELM-043 · `DESIGN` · `OPEN (v1a s50 · v1b s57)` · Condor health: short-strike proximity live (YELLOW 50% watch / 75% manage) + RED act-reason names tested side+depth (v1b); live greeks (steps 3-4) + breach→RED escalation (Gap B) remain**
...
**HELM-042 · `DESIGN` · `OPEN (v2 live s58)` · Re-base scan bias scorer: mean-reversion → momentum**
...
**HELM-050 · `OPS` · `OPEN` · Watchlist earnings cache under-refreshed (65/205)**
...
**s58:**
- **IVR coverage thin on liquid large-caps** — the unknown-IVR guard (shipped s58) surfaced that IVR is blank on many blue-chips: a s58 scan returned 5/6 missing (T/VZ/F/DIS/KHC), all liquid large-caps, not thin tickers. That is a data-source gap, not a scorer issue. Investigate why the IVR cache / `helm ivr refresh` (IBKR-sourced) is not covering liquid names; check whether it is staleness (like the HELM-050 earnings-cache cadence) or a coverage hole. Until resolved, expect a wall of "⚠ IVR missing" markers on affected scans.
- **Unknown IVR → flag, don't assess (HELM-042-adjacent)** — when a name has no IVR (no IBKR data), the scan must NOT route a strategy on the direction read alone. Keep the row visible, show "IVR missing — no assessment" where the strategy would go, and let the user run `helm ivr refresh` and re-scan. Rationale: HELM is fundamentally a volatility bet, so a trade chosen without the vol read is decided blind on the one input the system is built on; today `bias_to_strategy` silently falls back to a defined-risk trade that looks as legitimate as a fully-informed one. Same instinct as the health-side defer-on-non-live-marks rule. Scope: `bias_to_strategy` + scan display; keep the name in the ranked list, don't hide it.
