...
_Last updated: 2026-06-24 (s34 — WS4 prep shipped + wiring design locked · fork A · P2 mapping · verdict in `check_one` via `evaluate`+`band_for`, `assess_position`→evidence-only, SimpleNamespace adapter)._
...
- **Next highest-leverage:** **HELM-027 decision core** — WS0–WS3c shipped; WS4 prep shipped (`dte`→`helm.dates`, decision→check_cmd cycle broken; fork A locked — core is spine, renderers evidence-only); WS4 wiring **design locked** (fork A · P2 mapping · verdict in `check_one` via `evaluate`+`band_for` · `assess_position`→evidence-only · `generate_guidance` table-driven · SimpleNamespace adapter, `decision.py` untouched); next is the wiring **build** — read-pass of `check_one`/`assess_position`/`generate_guidance` + key-name check vs `evaluate`'s 7 attrs, then guarded patch. Closes the APLD verdict gap. Beyond it the OPEN backlog is essentially drained — what remains is deferred-by-design or RTH-paired: HELM-019 Part 2 (HELM `assess_position` vs the Fidelity oracle, divergence delta, RTH), HELM-004's remaining multileg legs (thin-name trigger), HELM-023 learning layer (corpus-gated). Parking desk picks: COVERED_CALL gradeability and the `helm status`/`helm checkpoint` CLI.
- **Last shipped (s34):** decision core WS4 prep — `dte` relocated to `helm.dates`, decision→check_cmd import cycle broken (frees `check` to consume the core), fork A locked. Prior (s33): WS3c — COVERED family (profit-on-credit, no stop) + DIAGONAL family (PMCC/DIAGONAL/DIAGONAL_PUT, calendar-only off the back/long leg, no profit/stop); DIAGONAL_PUT settings row seeded; calendar tail made family-aware. Prior (s32): WS2 (`helm/decision.py`) + WS3a/b (long/debit family routing) + WS1 levers + `_validate` guard.
...
- **Counts:** 6 active · 4 parked · last shipped s34 (WS4 prep — `dte`→`helm.dates`, decision→check_cmd cycle broken).
...
- **WS4** [prep done · s34] — `check` default → core verdict; `--deep` renderers demoted from deciders to evidence formatters. Prep shipped: `dte`→`helm.dates` (decision→check_cmd cycle broken, frees `check` to import the core). Fork A locked (core is spine; renderers/guidance become evidence-only). Remaining: verdict→display map + wire `check_one`/deep renderers to `decision.evaluate`.
