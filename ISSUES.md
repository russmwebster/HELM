...
_Last updated: 2026-06-24 (s34 — decision core WS4 prep: `dte`→`helm.dates`, decision→check_cmd cycle broken; fork A locked)._
...
- **Next highest-leverage:** **HELM-027 decision core** — WS0–WS3c shipped; WS4 prep shipped (`dte`→`helm.dates`, decision→check_cmd cycle broken; fork A locked — core is spine, renderers evidence-only); next is the WS4 wiring proper — verdict→display map + point real-book `check` at `decision.evaluate`, closing the APLD verdict gap. Beyond it the OPEN backlog is essentially drained — what remains is deferred-by-design or RTH-paired: HELM-019 Part 2 (HELM `assess_position` vs the Fidelity oracle, divergence delta, RTH), HELM-004's remaining multileg legs (thin-name trigger), HELM-023 learning layer (corpus-gated). Parking desk picks: COVERED_CALL gradeability and the `helm status`/`helm checkpoint` CLI.
...
- **Counts:** 5 active · 4 parked · last shipped s34 (WS4 prep — `dte`→`helm.dates`, decision→check_cmd cycle broken).
