...
- **Next highest-leverage:** `pnl_pct` corruption resolved (s48) — HELM-035 contract-multiplier denominator bug repaired both tails (1,460 rows), long-debit outcome labels now trustworthy; only 26 numerator-corrupt rows remain, contained under **HELM-045** (3 positions, 6/15–18). Live shadow/decision path recomputes from `net_premium`, never affected. Next: HELM-045 (recover-or-quarantine the 26), then HELM-035 forward fix (`check_cmd.py`, low urgency). Deliberate builds: **HELM-042** (momentum re-base), **HELM-043** (condor health greeks).
...
**HELM-045 · `DATA` · `OPEN` · `pnl_unrealized` exceeds max profit — 26 rows, 3 positions, 6/15–18 re-mark window**
_Split from HELM-035 (s48). Contained numerator anomaly, NOT the systemic denominator bug: MCD-IC-20260609-MANUAL (16), CAT-IRON_CONDOR-20260617-B81F91 (6), JPM-BEAR_CALL_SPREAD-20260617-09009B (4), all dated 2026-06-15..06-18 — the mid-June re-mark the s38 note first spotted. `pnl_unrealized` exceeds `max_profit`, so recompute is unsafe; these are the ONLY uncorrected `pnl_pct` rows, quarantined out of Pass 2 by the guard. Gates HELM-023 for these winners only. s24 invariant (persisted `pnl_pct` > 100% on credit → FAIL) confirmed valid. Action: pull fills/marks for the 3 positions across 6/15–18 — recover true `pnl_unrealized` if entry data survives, else flag-and-quarantine (exclude from corpus)._
