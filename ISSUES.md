...
_Last updated: 2026-07-02 (s46 — **HELM-031 shadow-capture built end-to-end** (helper `83fec91` · schema+migration `e6f396e` · REAL-check wiring `fb1a241`); `ORIENTATION.md` added (`20bd592`); `cots2` reference tree removed (`210ca8a`); charter trimmed in project Instructions. Continuity hardened — charter=Instructions · ORIENTATION.md=structure · ISSUES.md=status.)_
...
- **Next highest-leverage:** the s48 data-integrity arc is closed — HELM-035 (`pnl_pct` denominator, 1,460 rows), HELM-045 (26-row numerator quarantine + guard), HELM-046 (14 outcome-less paper positions removed), HELM-047 (firewall allowlist hardened) all resolved. Corpus is clean: closed paper 73/73 with outcomes, real book intact, `pnl_pct` trustworthy both tails. Frontier returns to the deliberate builds: **HELM-042** (mean-reversion → momentum bias re-base, staged paper validation) and **HELM-043** (condor health greeks). Low-urgency tail: HELM-035 forward fix (`check_cmd.py` fallback).
- **Last shipped (s47):** **HELM-044** — earnings surfaced at the entry decision, end-to-end. L2: gate-starvation fix so `next_earnings` populates on scan, cache live 65/65 active (`ec73a47`); L2b: passed+fresh re-fetch loop closed — FDX no longer burns a fetch slot every scan (`7379e96`); L1: `helm open` earnings banner across all strategies via the run() header (`9d2af86`); L1b: `helm scan` cell unified on `classify_earnings`, past split from unknown (`6644946`). All pushed to origin.
...
- **Counts:** 15 active (10 OPEN · 5 DEFERRED) · 7 parked · last shipped s47 (HELM-044). Origin current through `6644946` — 0 unpushed.
...
_Last updated: 2026-07-02 (s47 — **HELM-044 earnings-at-entry shipped end-to-end**: L2 cache/gate fix (`ec73a47`) · L2b re-fetch loop closed (`7379e96`) · L1 open banner (`9d2af86`) · L1b scan unify (`6644946`). Layers 1 & 2 closed; layer 3 (corpus) the remainder. Origin current, pushed through `6644946`.)_
