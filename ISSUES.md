...
_Last updated: 2026-07-01 (s44 — **HELM-041 WS5c shipped**: `check_cmd` real-book per-leg marking + `_core_band_ml` multi-leg `/health` band + card display, validated live off the 10:00 `leg_checks` pass (50 GOOD rows / 20 positions). Also HELM-033 narrowed — /health gathers now REAL-default, commingling closed for all strategies. Commit hashes pending backfill.)_
