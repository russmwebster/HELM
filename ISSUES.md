...
_Last updated: 2026-06-27 (Sat — RTH-only persistence + cadence locked. The paper manage pass becomes the **sole** snapshot writer, persisting at its existing 10:00 / 12:30 / 15:45 ET slots, live-marks-only; ad-hoc + `/health` read-only. (Weighed 1× close-anchored and 2× 11:00/15:00; Russ locked the existing 3 slots — no schedule change, no second-guessing.) Earlier today: IB off-hours probe (frozen greeks ARE available, caveats noted), full-book triage off Fidelity (16 positions, −$50.6k unrealized; WELL closed; deep-ITM spec CSPs = willing-to-own calls), and flipped `stop_ab_active` → '1' (HELM-030 stop A/B now live; resolves the s37 pending item). Next: build HELM-036 accessor + HELM-037 writer together.)._
