...
- **Next highest-leverage:** `pnl_pct` corruption fully resolved (s48) — HELM-035 (denominator, both tails, 1,460 rows) and HELM-045 (26-row numerator anomaly, quarantined + guarded) both closed; long-debit outcome labels trustworthy. Frontier now **HELM-046** — 14 CLOSED positions missing `realized_pnl` (6/17 cohort), a bigger corpus hole than the marks were. Then the HELM-035 forward fix (`check_cmd.py` fallback, low urgency). Deliberate builds: **HELM-042** (momentum re-base), **HELM-043** (condor health greeks).
...
**HELM-046 · `DATA` · `OPEN` · 14 CLOSED positions missing `realized_pnl` (6/17 cohort)**
_Surfaced s48 during HELM-045. 14 of 119 CLOSED positions have `realized_pnl = NULL` — all in the 2026-06-17 open cohort (AMZN-BPS, BRK-B-IC, CAT-IC, CVX-IC, FCX-IC, GOOGL-BPS, JNJ-CSP, JPM-BCS, …). Closed trades with no outcome label are holes in the HELM-023 corpus, and more consequential than HELM-045's 26 marks (outcomes vs intra-life marks). Likely the close-path didn't stamp `realized_pnl` for that batch, or the same mid-June re-import artifact. Action: characterize the cohort — why the stamp is missing and whether `realized_pnl` is reconstructable from fills/close marks; else flag the positions so HELM-023 skips them._
