...
_Logged 6/29. `helm open <TICKER> <SPREAD> --confirm` runs the full scan + recommendation, but `confirm_spread()` (open_cmd.py:892) only prints "coming soon" and returns — no DB write. Branch is `config["is_spread"]`, so it hits every spread: BEAR_CALL / BULL_PUT / IRON_CONDOR / BEAR_PUT. Single-leg opens persist via `open_position_with_snapshot(contract=selected)`, which is single-contract-shaped — that is why multi-leg was deferred. Workaround: hand-log via `helm activity` after filling in Fidelity. Impact: spread opens bypass the entry-snapshot path that stamps the originating signal/scan_data, so they do not auto-flow into the counterfactual corpus — bears on HELM-012 (signal stamping) and HELM-023 (the learning layer grading autonomous picks vs live decisions; spread picks would be blurred). Data model already supports multi-leg (positions ↔ legs; gather_icondor/gather_bearput read them), so this is persistence wiring, not a schema change — needs an N-leg open_position_with_snapshot path with per-leg entry snapshots. Pairs with HELM-029 (canonical Position/Leg). Interim question: should hand-logged spreads still carry a signal stamp so the corpus is not blind to HELM-originated spread picks?_
