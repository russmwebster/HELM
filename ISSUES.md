...
_Last updated: 2026-07-03 (s50 — **HELM-043 v1a shipped**: condor short-strike proximity signal (greeks-free) folds into the condor `/health` band — a tested short strike now lifts green→YELLOW (prep 50% / act 75%). Validated live on the REAL trio: LRCX green→YELLOW "short put tested (116%)" (the s45 false-green), AMD watch (58%), PG green (47%). HELM-043 stays OPEN — v1b (RED act-reason) + greeks (3-4) remain. Origin at `51c385c`; this checkpoint commit follows.)_
...
- **Next highest-leverage:** **HELM-043 v1b** — promote the ≥75% "tested" tier from YELLOW to a RED **act reason** in the decision core (`decision.py`), so `/health` and the paper auto-manager both treat a breached short strike as an action state (evidence caps at YELLOW by design). Touches the shared verdict → sequences near **HELM-027**. Then **HELM-042** (mean-reversion → momentum bias re-base). Greeks steps (3-4) stay gated on the IBKR feed.
- **Last shipped (s50):** HELM-043 v1a — condor short-strike proximity signal (greeks-free). `verdict.py`: `condor_proximity()` + PROX_PREP/ACT + a multi-leg YELLOW-lift in `band_for`; `health.py` `_core_band_ml` reads the short strikes + latest journaled `checks.spot_price` and folds proximity into the verdict evidence. Validated live on the REAL trio — LRCX green→YELLOW "short put tested (116%), manage" (the s45 false-green), AMD watch (58%), PG green (47%). 16 fixtures green.
...
- **Counts:** 16 active (10 OPEN · 6 DEFERRED) · last shipped s50 · origin at `51c385c` (this checkpoint commit follows).
