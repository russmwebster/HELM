...
_Last updated: 2026-06-27 (Sat — RTH-only scope decision. Ran a read-only IB Gateway off-hours probe: frozen marks + full model greeks ARE available off-hours (7/8 legs) with caveats (`undPrice` beats the stock quote, `close` field stale, per-contract `marketDataType` varies). Russ then scoped HELM to **live-RTH-only persistence** — dropped the off-hours frozen-snapshot path from HELM-037; writer = one RTH-anchored live mark per position per day, ad-hoc + `/health` read-only. Also triaged the full book off a Fidelity download (16 positions, −$50.6k unrealized ≈ 5.6% of IRA; WELL booked closed; four deep-ITM spec CSPs flagged as willing-to-own decisions). STILL PENDING from s37: flip `stop_ab_active` → '1'. Next: build HELM-036 accessor + HELM-037 RTH writer together.)._
